...
export async function generatePanelImage(panelDescription, characterContext, style, seed, lora) {
  ...
  // CRITICAL ORDERING: styleModifier + QUALITY run 250-450+ chars on their own (e.g. PRISMA's style
  // string alone is ~250 chars and literally contains the phrase "expressive anime manga faces") — the
  // server-side proxies (api/image.js, api/deepinfra.js) truncate the incoming prompt to avoid provider
  // 400s. Putting style/quality FIRST meant truncation cut the prompt before it ever reached
  // ${desc}/${chars} — every panel rendered as a generic face close-up regardless of the actual scene.
  // Scene + characters + action now come first so they survive truncation; style/quality trail.
  // A busy panel (long scene + multiple characters) can still run 1800+ chars BEFORE styleModifier even
  // starts, so a short ESSENTIAL tag (color mode + anatomy + no-text) is placed right up front too —
  // guaranteed to survive any truncation regardless of how much scene/character text precedes it. This
  // is what was causing SOME panels to render black-and-white / inconsistent style: on busier panels the
  // color and style cues were silently truncated away entirely, while lighter panels kept them.
  const essential = isMono
...
- PHYSICAL ACTION AND ADVENTURE ARE NOT OPTIONAL: a chapter needs the cast actually DOING things in the world, not just discussing them — descending into a dungeon, crossing dangerous terrain, being ambushed or attacked by a monster/enemy, chasing or fleeing, a physical challenge or discovery. If the world/genre supports danger, monsters, factions, or a physical quest (murim, xianxia, dungeon/isekai, horror, action — check the world and genre conventions), put the cast bodily INTO that danger on-page every chapter, don't just have them talk about it. Aim for a real mix across the chapter: some panels are people talking, but several must be people moving, exploring, fighting, or reacting to a physical threat.
...
DIALOGUE — a major engine of the chapter, alongside physical action (see PHYSICAL ACTION AND ADVENTURE above — don't let talk crowd out real events). Once characters are together (by panel 3), conversation carries the scenes that ARE conversations. Make those exchanges FLOW naturally:
