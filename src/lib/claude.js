...
export async function generatePanelImage(panelDescription, characterContext, style, seed, lora) {
  ...
  // server-side proxies (api/image.js, api/deepinfra.js) truncate the incoming prompt to a few hundred
  // chars to avoid provider 400s. Putting style/quality FIRST meant that truncation cut the prompt
  // before it ever reached ${desc}/${chars} — every panel was generated from nothing but the style
  // preamble, which is exactly why panels rendered as generic face close-ups regardless of the actual
  // scene. Scene + characters + action now come first so they survive truncation; style/quality trail.
